...
export const getUser = actionClient
  ...
  .action<ReturnActionType>(async ({ parsedInput }) => {
    ...
    const filteredUser = {
      ...
      isFollowing:
        user?.followers?.includes(session?.currentUser?._id) || false,
    };
...
export const getNotifications = actionClient
  ...
  .action<ReturnActionType>(async ({ parsedInput }) => {
    ...
    const session = await getServerSession(authOptions);
    if (!session)
      return { failure: "You need to be logged in to view notifications" };
    const notifications = await Notification.find({ user: postId }).sort({
      createdAt: -1,
    });
    await User.findByIdAndUpdate(session.currentUser?._id, {
      $set: { hasNewNotifications: false },
    });
    return JSON.parse(JSON.stringify({ notifications }));
  });

export const getFollowUser = actionClient
  .schema(getFollowSchema)
  .action<ReturnActionType>(async ({ parsedInput }) => {
    await connectToDatabase();
...
export const getUserPosts = actionClient
  .schema(idSchema)
  .action<ReturnActionType>(async ({ parsedInput }) => {
    const { id } = parsedInput;
    await connectToDatabase();
...
export const unfollow = actionClient
  .schema(followsSchema)
  .action<ReturnActionType>(async ({ parsedInput }) => {
    await connectToDatabase();
    const { userId, currentUserId } = parsedInput;
...
export const deleteNotifications = actionClient
  .schema(idSchema)
  .action<ReturnActionType>(async ({ parsedInput }) => {
    const { id } = parsedInput;
    await connectToDatabase();
