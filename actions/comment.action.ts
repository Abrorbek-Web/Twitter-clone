...
export const createComment = actionClient
  ...
  .action<ReturnActionType>(async ({ parsedInput }) => {
    ...
    const session = await getServerSession(authOptions);
    if (!session)
      return { failure: "You must be logged in to create a comment." };
...
export const likeComment = actionClient
  ...
  .action<ReturnActionType>(async ({ parsedInput }) => {
    ...
    const session = await getServerSession(authOptions);
    if (!session)
      return { failure: "You must be logged in to like a comment." };
    const comment = await Comment.findByIdAndUpdate(id, {
      $push: { likes: session.currentUser?._id },
    });
    await Notification.create({
...
export const unlikeComment = actionClient
  .schema(idSchema)
  .action<ReturnActionType>(async ({ parsedInput }) => {
    const { id } = parsedInput;
    await connectToDatabase();
    const session = await getServerSession(authOptions);
    if (!session)
      return { failure: "You must be logged in to unlike a comment." };
    const comment = await Comment.findByIdAndUpdate(id, {
