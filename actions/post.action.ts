...
export const likePost = actionClient
  ...
  .action<ReturnActionType>(async ({ parsedInput }) => {
    ...
    if (currentPost.likes.includes(session.currentUser?._id))
      return { failure: "You have already liked this post." };
...
export const deleteLike = actionClient
  .schema(idSchema)
  .action<ReturnActionType>(async ({ parsedInput }) => {
    const { id } = parsedInput;
    const session = await getServerSession(authOptions);
    ...
    if (!currentPost) return { failure: "Post not found." };
    if (!currentPost.likes.includes(session.currentUser?._id))
      return { failure: "You have not liked this post." };
    await Post.findByIdAndUpdate(id, {
